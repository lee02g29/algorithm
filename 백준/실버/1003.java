...
class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 2; i <= 40; i++) { // 2부터 40까지 미리 계산하기
			cnt_fibo_1[i] = cnt_fibo_1[i-1] + cnt_fibo_1[i-2]; // i번째의 1 호출 횟수 = i-1의 1 호출 횟수 + i-2의 1 호출 횟수
			cnt_fibo_0[i] = cnt_fibo_0[i-1] + cnt_fibo_0[i-2]; // i번째의 0 호출 횟수 = i-1의 0 호출 횟수 + i-2의 0 호출 횟수
		}
